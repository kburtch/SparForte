------------------------------------------------------------------------------
-- BUSH AdaScript Language Parser                                           --
--                                                                          --
-- Part of BUSH                                                             --
------------------------------------------------------------------------------
--                                                                          --
...
--                                                                          --
------------------------------------------------------------------------------
-- CVS: $Header: /home/cvsroot/bush/src/parser.adb,v 1.6 2005/08/31 15:10:44 ken Exp $
     ...
     if baseTypesOk( kind1, kind2 ) then
        ...
        if operation = uni_numeric_t or else operation = root_enumerated_t or else operation = cal_time_t then
             ...
             if operator = ">=" then
                ...
                err( "in must be used with scalar types" );
             elsif operator = "not in" then
                err( "not in must be used with scalar types" );
             else
                err( "Internal error: couldn't handle relational operator" );
             end if;
             if b then
...
begin
  ...
  if isExecutingCommand then
     ...
     when peek =>
        ...
        -- put_line( getStackTrace ); exists in Bush 2.0
     when noCommandHash =>
        clearCommandHash;
     ...
     when template | unrestricted_template =>
        ...
        if var_id = eof_t  then
           ...
           if length( templatePath ) > 5 and then tail( templatePath, 5 ) = ".bush" then
              -- Delete( templatePath, length(templatePath)-4, 6 );
...
begin
  ...
  -- (Constant assignments, etc. occur only when actually running a script)
   ...
   if identifiers( elementBaseType ).list  then
      ...
      if isExecutingCommand or syntax_check then            -- not on synchk
         declareArrayType( id => type_id,
   ...
   if Token = symbol_t and identifiers( token ).value = "(" then
      ...
      while token /= eof_t loop                            -- name [,name]
         ...
         if isExecutingCommand or syntax_check then        -- OK to do it?
            ...
            identifiers( newtype_id ).value := to_unbounded_string( enum_index'img );
...
begin
  if inputMode = breakout then
     ...
     expect( return_t );
     if token /= eof_t and token /= symbol_t and identifiers( token ).value /= ";" and identifiers( token ).value /= "|" then
        if isExecutingCommand then
...
begin
  loop                                        -- repeatedly
    ...
    if length( command ) > 0 then                       -- something there?
       ...
       sourceFilesList.Push( SourceFiles, aSourceFile'( name => to_unbounded_string( commandLineSource ) ) );
...
begin
  ...
  if execOpt then                                  -- -e?
     ...
     -- put( ASCII.ESC & "]2;" & "bush" & ASCII.BEL  ); -- xterm window title
     sourceFilesList.Clear( SourceFiles );
     sourceFilesList.Queue( SourceFiles, aSourceFile'(name => to_unbounded_string (commandLineSource) ) );
    ...
    sourceFilesList.Clear( SourceFiles );
    sourceFilesList.Queue( SourceFiles, aSourceFile'(name => scriptFilePath) );
...
begin
  ...
  scriptFilePath := to_unbounded_string( commandLineSource ); -- "script" name
  sourceFilesList.Clear( SourceFiles );
  sourceFilesList.Push( SourceFiles, aSourceFile'( name => basename( scriptFilePath ) ) );
...
begin
  if optionOffset = 0 then                       -- no arguments or '-'?
     if isLoginShell then                        -- login shell? find profile
        ...
        -- we're using scriptFile and scriptFilePath as temp variables
        -- here.  They'll be overwritten when the script is interpreted.
        scriptFile := open( "/etc/bush_profile" & ASCII.NUL, 0, 660 ); -- open script
        if scriptFile > 0 then                       -- good?
        ...
        if home_id /= eof_t then                     -- HOME defined?
           ...
           scriptFilePath := scriptFilePath & ".bush_profile";
     ...
     if syntaxOpt then                                  -- -c / --check?
        ...
        interpretScript( Argument( OptionOffset ) );    -- check the script
