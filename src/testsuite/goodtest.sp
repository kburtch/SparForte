...
-- Single-quote test
s := "'a', 'b', 'c'";
strings.csv_replace( s, 2, "0", ',', true);
pragma assert( s = "'a',0, 'c'" );
s := '"' & "a" & '"' & ',' & '"' & "b" & '"' & "," & '"' & "c" & '"';
strings.csv_replace( s, 2, "0", ',', false);
pragma assert( s = ASCII.Quotation & "a" & ASCII.Quotation & ",0," & ASCII.Quotation & "c" & ASCII.Quotation );
s := "'a', 'b', 'c'";
strings.csv_replace( s, 1, "0",',', false);
pragma assert( s = "0, 'b', 'c'" );
s := '"' & "a" & '"' & ',' & '"' & "b" & '"' & "," & '"' & "c" & '"';
strings.csv_replace( s, 2, "0", ',', false);
pragma assert( s = ASCII.Quotation & "a" & ASCII.Quotation & ",0," & ASCII.Quotation & "c" & ASCII.Quotation );
s := "'a', " & '"' & "'b', 'c'" & '"';
strings.csv_replace( s, 2, "0", ',', false);
pragma assert( s = "'a',0" );
s := ASCII.Quotation & "a" & ASCII.Quotation & ", '" & ASCII.Quotation & 'b' & ASCII.Quotation & ", " & ASCII.Quotation & "c" & ASCII.Quotation & "'";
strings.csv_replace( s, 2, "0", ',', true);
pragma assert( s = ASCII.Quotation & "a" & ASCII.Quotation & ",0" );
...
n := strings.levenshtein( "kitten", "sitting" );
pragma assert( n = 3 );
n := strings.levenshtein( "rosettacode", "raisethysword" );
pragma assert( n = 8 );
n := strings.levenshtein( "rosetta", "rosetta" );
pragma assert( n = 0 );
s := strings.soundex( "Soundex" );
pragma assert( s = "S532" );
s := strings.soundex( "Lee" );
pragma assert( s = "L000" );
s := strings.soundex( "Ashcraft" );
pragma assert( s = "A261" );
s := strings.soundex( "" );
pragma assert( s = "" );
s := strings.replace_all( "foobar", "o", "X", sensitivity.sensitive );
pragma assert( s = "fXXbar" );
s := strings.replace_all( "foobar", "O", "X", sensitivity.sensitive );
pragma assert( s = "foobar" );
s := strings.replace_all( "foobar", "O", "X", sensitivity.insensitive );
pragma assert( s = "fXXbar" );
s := strings.replace_all( "foobar", "f", "X", sensitivity.sensitive );
pragma assert( s = "Xoobar" );
s := strings.replace_all( "foobar", "r", "X", sensitivity.sensitive );
pragma assert( s = "foobaX" );
s := strings.replace_all( "foobar", "oo", "X", sensitivity.sensitive );
pragma assert( s = "fXbar" );
s := strings.replace_all( "foobar", "foobar", "X", sensitivity.sensitive );
pragma assert( s = "X" );
s := strings.replace_all( "", "foobar", "X", sensitivity.sensitive );
pragma assert( s = "" );
s := strings.replace_all( "foobar", "o", "X" );
pragma assert( s = "fXXbar" );
s := strings.replace_all( "foobar", "O", "X" );
pragma assert( s = "foobar" );
s := strings.replace_all( "foobaroozle", "oo", "_" );
pragma assert( s = "f_bar_zle" );
s := strings.replace_all( "foobaroozle", "oo", "_", sensitivity.insensitive );
pragma assert( s = "f_bar_zle" );
b := strings.starts_with( "foobar", "foo", sensitivity.sensitive );
pragma assert( b = true );
b := strings.starts_with( "foobar", "FOO", sensitivity.sensitive );
pragma assert( b = false );
b := strings.starts_with( "foobar", "baz", sensitivity.sensitive );
pragma assert( b = false );
b := strings.starts_with( "foobar", "foo", sensitivity.insensitive );
pragma assert( b = true );
b := strings.starts_with( "foobar", "FOO", sensitivity.insensitive );
pragma assert( b = true );
b := strings.starts_with( "foobar", "baz", sensitivity.insensitive );
pragma assert( b = false );
b := strings.starts_with( "foobar", "foo" );
pragma assert( b = true );
b := strings.starts_with( "foobar", "FOO" );
pragma assert( b = false );
b := strings.starts_with( "foobar", "baz" );
pragma assert( b = false );
b := strings.ends_with( "foobar", "bar", sensitivity.sensitive );
pragma assert( b = true );
b := strings.ends_with( "foobar", "BAR", sensitivity.sensitive );
pragma assert( b = false );
b := strings.ends_with( "foobar", "baz", sensitivity.sensitive );
pragma assert( b = false );
b := strings.ends_with( "foobar", "bar", sensitivity.insensitive );
pragma assert( b = true );
b := strings.ends_with( "foobar", "BAR", sensitivity.insensitive );
pragma assert( b = true );
b := strings.ends_with( "foobar", "baz", sensitivity.insensitive );
pragma assert( b = false );
b := strings.ends_with( "foobar", "bar" );
pragma assert( b = true );
b := strings.ends_with( "foobar", "BAR" );
pragma assert( b = false );
b := strings.ends_with( "foobar", "baz" );
pragma assert( b = false );
n := strings.word_count( "" );
pragma assert( n = 0 );
n := strings.word_count( "a" );
pragma assert( n = 1 );
n := strings.word_count( "ab" );
pragma assert( n = 1 );
n := strings.word_count( "a1b" );
pragma assert( n = 2 );
n := strings.word_count( "a b" );
pragma assert( n = 2 );
n := strings.word_count( "a b " );
pragma assert( n = 2 );
n := strings.word_count( " a b" );
...
pragma assert( i < 0 );
i := strings.compare( "a", "a" );
pragma assert( i = 0 );
i := strings.compare( "b", "a" );
pragma assert( i > 0 );
i := strings.compare( "a", "a", sensitivity.sensitive );
pragma assert( i = 0 );
i := strings.compare( "a", "A", sensitivity.sensitive );
pragma assert( i = 0 );
i := strings.compare( "a", "A", sensitivity.insensitive );
pragma assert( i = 1 );
i := strings.compare( "foobar", "foobaz", sensitivity.insensitive, 3 );
pragma assert( i = 0 );
n := strings.index_set( "marketplace", "place", 1, membership.inside );
pragma assert( n = 2 );
n := strings.index_set( "marketplace", "mark", 1, membership.outside );
pragma assert( n = 5 );
n := strings.index_set( "marketplace", "aeiou", 11, membership.outside, direction.backward );
pragma assert( n = 10 );
n := strings.index_set( "marketplace", "z", 1 );
pragma assert( n = 0 );
n := strings.index_set( "marketplace", "", 1 );
pragma assert( n = 0 );
n := strings.index_set( "", "", 1 );
pragma assert( n = 0 );
...
begin
   mycase( customer_loyalty, rebate );
...
declare
   ...
   outage_duration  : a_outage_duration := long;
   rebate           : a_rebate;
...
begin
   mycase( customer_loyalty, outage_duration, rebate );
...
declare
   ...
   type a_rebate is (none, small, medium, large);
   customer_loyalty : a_customer_loyalty := loyal;
   outage_duration  : a_outage_duration := short;
   rebate           : a_rebate;
   rebate_days      : natural;
...
begin
   mycase( customer_loyalty, outage_duration, rebate, rebate_days );
   pragma assert( rebate = medium );
